...
const PlayerAvatar = ({ name, active, isBot, position, cardsCount, mdcPoints, wins, isBoude, chatMessage, isVip, equippedAvatar }) => {
    ...
    const originClass = position.includes('left') ? 'origin-top-left' : 'origin-top-right';

    return (
        <div className={`absolute flex gap-2 md:gap-4 transition-all duration-300 items-center ${active ? 'scale-105 opacity-100 z-[100]' : 'opacity-80 scale-100'} scale-75 md:scale-100 ${originClass}`} style={style}>
            {chatMessage && (
...
        <button className="p-2 bg-zinc-800 text-zinc-400 hover:text-white rounded-full transition-colors"><Settings size={20} /></button>
      </div>
    </div>
  );
};
...
  const [winningInfo, setWinningInfo] = useState(null); // { winnerId, winningTile }
  // MODIFICATION: suppression de l'état orientation car plus d'overlay de blocage
...
         <div className="w-full h-full flex items-center justify-center pointer-events-none relative overflow-hidden z-10">
            <div ref={boardRef} className="flex items-center justify-center origin-center drop-shadow-[0_30px_60px_rgba(0,0,0,0.9)] whitespace-nowrap" style={{ transform: `scale(${zoomScale})` }}>
